...
awesome.register_xproperty("handy2_id", "string")
awesome.register_xproperty("handy2_visible", "boolean")

local function spawn_callback(handy2_id, placement, options, screen)
	return function(c)
		if clients[screen] == nil then clients[screen] = {} end
		c:set_xproperty("handy2_id", handy2_id)
		clients[screen][handy2_id] = c
...
local function toggle_client(c, s)
	if c:isvisible() then
		c.hidden = true
		c:set_xproperty("handy2_visible", false)
	else
		c:move_to_tag(s.selected_tag)
		client.focus = c
		c.hidden = false
		c:set_xproperty("handy2_visible", true)
	end
end

-- look for an already running client on a single screen
local function restore_client_single_screen(handy2_id, s, key, properties, target_screen)
	if not target_screen then
		target_screen = s
	end

	for _,c in ipairs(s.all_clients) do
		if c:get_xproperty("handy2_id") == handy2_id then
			clients[key][handy2_id] = c
			c:connect_signal("unmanage", function (c)
				clients[key][handy2_id] = nil
			end)
...
local function restore_client(handy2_id, key, properties, target_screen)
	if key == 'single' then
		-- try all screens for single instance clients
		for s in screen do
			if restore_client_single_screen(handy2_id, s, key, properties, target_screen) then
				return true
			end
		end
		return false
	else
		return restore_client_single_screen(handy2_id, target_screen, key, properties, target_screen)
	end
end
